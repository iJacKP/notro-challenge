...
describe('SearchRepositoriesUseCase', () => {
  ...
  it('✅ Deve retornar dados do cache quando disponível', async () => {
    const mockData: Repository[] = [{ name: 'repo1', url: '', description: '', stars: 0, watchers: 0, issues: 0 }];
  ...
  it('✅ Deve buscar dados da API quando não está no cache e salvar no cache', async () => {
    const mockData: Repository[] = [{ name: 'repo2', url: '', description: '', stars: 10, watchers: 5, issues: 1 }];
    ...
    expect(result).toEqual(mockData);
    expect(mockCacheManager.get).toHaveBeenCalledWith('repos:nestjs:1');
    expect(mockGithubAdapter.searchRepositories).toHaveBeenCalledWith('nestjs', 1);
    expect(mockCacheManager.set).toHaveBeenCalledWith('repos:nestjs:1', mockData, 300000);
  });
});